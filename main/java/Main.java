...
public class Main {
    ...
    public static void GotoStore() {
        int input = -1;
        while (input != 5) {
            System.out.println("Выберите действие: 1/Вывести список всех товаров, 2/Поиск, 3/Добавить в корзину,4/Перейти в корзину 5/Выйти из магазина");
            input = sc.nextInt();
    ...
    public static void GotoBasket() {
        ...
        if (input.equals("y")) {
            deleteFromBacket();
            List<ProductItem> basketItems = basket.getItems();
            if (basketItems.size() != 0) {
                System.out.println("Товары в корзине:");
                Basket.outputListProduct(basketItems);
            } else {
    ...
    public static void addToBasket() {
        ...
        ProductItem item = store.getProductItemByProductId(id);
        if (item != null) {
            if (countProduct <= item.count) {
                basket.addItem(item.product, countProduct);
                Basket.outputListProduct(basket.getItems());
            } else {
